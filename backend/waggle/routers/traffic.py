...
async def _aggregated_traffic(
    ...
    result = await session.execute(
        text(
            ...
            f"CASE WHEN SUM(period_ms) > 0 "
            f"THEN SUM(bees_in) * 60000.0 / SUM(period_ms) "
            f"ELSE 0.0 END AS avg_bees_in_per_min, "
            f"CASE WHEN SUM(period_ms) > 0 "
            f"THEN SUM(bees_out) * 60000.0 / SUM(period_ms) "
            f"ELSE 0.0 END AS avg_bees_out_per_min "
            f"FROM bee_counts WHERE {where_sql} "
