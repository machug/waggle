...
class AlertEngine:
    ...
    async def check_reading(
        ...
        async with AsyncSession(self.engine) as session:
            ...
            swarm_result = await self._check_swarm_correlation(
                session, hive_id, reading
            )
    ...
    def _iso_cutoff(reading_time: datetime, hours: int) -> str:
        ...
        return (reading_time - timedelta(hours=hours)).strftime(
            "%Y-%m-%dT%H:%M:%S.%f"
        )[:-3] + "Z"
    ...
    def _parse_observed_at(observed_at: str) -> datetime:
        ...
        return datetime.strptime(observed_at, "%Y-%m-%dT%H:%M:%S.%fZ").replace(
            tzinfo=UTC
        )
    ...
    async def _check_low_activity(
        ...
        today_start = reading_time.replace(
