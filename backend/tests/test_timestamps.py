...
def test_utc_now_is_utc():
    ...
    result = utc_now()
    # Parse it back
    dt = datetime.strptime(result, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=UTC)
...
def test_validate_observed_at_too_old():
    ...
    old = (datetime.now(UTC) - timedelta(hours=73)).strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    )[:-3] + "Z"
...
def test_validate_observed_at_within_skew():
    ...
    recent = (datetime.now(UTC) - timedelta(hours=71)).strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    )[:-3] + "Z"
